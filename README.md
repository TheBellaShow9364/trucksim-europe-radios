# 🚛 TruckSim Europe Radios 🎵 

![Version](https://img.shields.io/badge/version-0.1--beta-orange) ![ETS2](https://img.shields.io/badge/ETS2-compatible-green)  ![ATS](https://img.shields.io/badge/ATS-compatible-green)  ![Language](https://img.shields.io/badge/radios-FR-blue)

**TruckSim Europe Radios** est une collection de stations radio françaises pour :

👉 Euro Truck Simulator 2        
👉 American Truck Simulator 

🎧 Profite de **100 radios françaises prêtes à l’emploi** pour une immersion totale.

## 📥 Téléchargement

👉 **Version actuelle : v0.1 (bêta)**  
[Télécharger la dernière version](https://github.com/TheBellaShow9364/trucksim-europe-radios/releases/latest)

👉 **Version actuelle : v0.2 (bêta)
En cours de développement (Pas disponible pour le moment)

---

## 🛠️ Installation
1. Télécharger le fichier  
2. Ouvrir le dossier Documents  
3. Aller dans : Euro Truck Simulator 2  ou American Truck Simulator  
4. Copier le fichier dans ce dossier  
5. Lancer le jeu  
6. Appuyer sur la touche R pour accéder aux radios

---

## ⚠️ Version bêta

Des améliorations et nouvelles radios arrivent bientôt !
